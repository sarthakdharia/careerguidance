<?php
  ...
  if(isset($_SESSION['key']))
  {
    if(@$_GET['q']== 'addquiz' && $_SESSION['key']=='suryapinky') 
    {
      ...
      $No = $_POST['No'];
      $id=uniqid();
      $q3=mysqli_query($con,"INSERT INTO quiz VALUES  ('$id','$name' , '$Yes' , '$No','$total', NOW())");
  ...
  if(@$_GET['q']== 'quiz' && @$_GET['step']== 2) 
  {
    ...
    if($ans == $ansid)
    {
      ...
      $r++;
      $s=$s+$Yes;
      $q=mysqli_query($con,"UPDATE `history` SET `score`=$s,`level`=$sn,`Yes`=$r, date= NOW()  WHERE  email = '$email' AND eid = '$eid'")or die('Error124');
    } 
    else
    {
      ...
      $w++;
      $s=$s;
      $q=mysqli_query($con,"UPDATE `history` SET `score`=$s,`level`=$sn,`No`=$w, date=NOW() WHERE  email = '$email' AND eid = '$eid'")or die('Error147');
    }
    if($sn != $total)
    {
      $sn++;
